...
// ── Fallback unavailability (used when no schedule exists yet) ─────────────────
const RENT_BOOKED_OFFSETS = new Set([3, 8, 11, 17, 22, 25]);
function fallbackRentUnavailable(d: Date): boolean {
  const s = sod(d);
...
function fallbackSvcUnavailable(d: Date): boolean {
  const s = sod(d);
  if (s < DEMO_TODAY) return true;
  if (s.getDay() === 0) return true;
  return SVC_BOOKED_OFFSETS.has(daysBetween(DEMO_TODAY, s));
}

// ── Schedule-aware unavailability checker ──────────────────────────────────────
function makeUnavailabilityChecker(schedule: ListingSchedule | null, fallback: (d: Date) => boolean) {
  if (!schedule) return fallback;
  return (d: Date): boolean => {
    const s = sod(d);
    if (s < DEMO_TODAY) return true;
    return getSlotStatus(schedule, d) !== "available";
  };
}
  ...
  return (
    <div className="select-none">
      <div className="flex items-center justify-between mb-3">
        <button type="button" onClick={onPrevMonth} className="w-8 h-8 rounded-full flex items-center justify-center text-stone-400 dark:text-stone-500 hover:bg-stone-200 dark:hover:bg-[#252837] transition-colors">
          <ChevronLeft className="w-4 h-4" />
        </button>
        <span className="text-sm font-bold text-stone-900 dark:text-stone-50">
          {MONTH_NAMES[viewMonth]} {viewYear}
        </span>
        <button type="button" onClick={onNextMonth} className="w-8 h-8 rounded-full flex items-center justify-center text-stone-400 dark:text-stone-500 hover:bg-stone-200 dark:hover:bg-[#252837] transition-colors">
      ...
      <div className="grid grid-cols-7">
        {cells.map((d, i) => {
          if (d.getMonth() !== viewMonth) return <div key={i} className="h-9" />;
          ...
          return (
            <div key={i} className="relative h-9 flex items-center justify-center">
              ...
              <button
                ...
                className={cn(
                  "relative z-10 w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium transition-all duration-100",
                  isPast  && "text-stone-300 dark:text-stone-700 cursor-not-allowed",
                  unavail && !isPast && ["text-red-300 dark:text-red-900/60", "line-through cursor-not-allowed"],
                  !disabled && !isSelected && ["text-stone-700 dark:text-stone-200 cursor-pointer", colors.hoverBg],
                  isSelected && ["font-bold text-white", colors.solid],
                  isToday && !isSelected && ["ring-2 ring-offset-1", colors.ringToday, "ring-offset-stone-50 dark:ring-offset-[#13151f]"],
                )}
              >
                {d.getDate()}
              </button>
...
// ── Schedule availability info banner ─────────────────────────────────────────
function ScheduleInfoBanner({ schedule }: { schedule: ListingSchedule }) {
  const dayNames = ["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];
  const opDays = schedule.operatingDays.map(d => dayNames[d]).join(", ");
  return (
    <div className="flex items-start gap-2 bg-stone-50 dark:bg-[#13151f] border border-stone-200 dark:border-[#2a2d3e] rounded-xl px-3 py-2.5">
      <Clock className="w-3.5 h-3.5 text-stone-400 shrink-0 mt-0.5" />
      <p className="text-[11px] text-stone-500 dark:text-stone-400 leading-relaxed">
        Available {opDays} · {schedule.operatingStart}–{schedule.operatingEnd}
        {" · "}Book up to {schedule.advanceBookingDays} days ahead
      </p>
    </div>
  );
}
...
export function RequestToRentModal({
  open, onClose, listingId, listingTitle, listingPrice, priceUnit, buyerName = "Guest",
}: RequestToRentModalProps) {
  const { getSchedule, createBooking } = useSchedule();
  const schedule = getSchedule(listingId);
  const isUnavailable = makeUnavailabilityChecker(schedule, fallbackRentUnavailable);
  ...
  function handleSelectDay(d: Date) {
    ...
    if (!startDate || bothSet) {
      setStartDate(sod(d)); setEndDate(null);
    } else {
  ...
  const estimatedTotal = (() => {
    ...
    if (unit.includes("night") || unit.includes("day")) return listingPrice * numNights;
    if (unit.includes("week"))  return Math.round((listingPrice / 7) * numNights);
    if (unit.includes("month")) return Math.round((listingPrice / 30) * numNights);
    return null;
  })();
  ...
  function handleSend() {
    ...
    // If schedule exists, book the start date (representative)
    if (schedule) {
      const booked = createBooking(listingId, startDate, buyerName, undefined, message || undefined);
      if (!booked) {
        toast.error("That date is no longer available. Please pick another.", { position: "top-center" });
        setSending(false);
        return;
      }
    }
    ...
    >
      <div className="bg-white dark:bg-[#1c1f2e] rounded-t-3xl sm:rounded-2xl w-full sm:max-w-md shadow-2xl flex flex-col max-h-[92vh] sm:max-h-[88vh]">
        ...
        <div className="bg-[#1e2433] px-5 py-4 flex items-start justify-between flex-shrink-0 rounded-t-3xl sm:rounded-t-2xl">
          ...
          <button type="button" onClick={onClose} className="text-slate-400 hover:text-white transition-colors ml-3 flex-shrink-0 mt-0.5">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="p-5 flex flex-col gap-5">
            ...
            {schedule && <ScheduleInfoBanner schedule={schedule} />}

            {/* Calendar */}
            <div>
              <div className="flex items-center justify-between mb-3">
                <p className="text-[11px] font-bold text-stone-500 dark:text-stone-400 uppercase tracking-widest">Select Dates</p>
                {(startDate || endDate) && (
                  <button type="button" onClick={handleClear} className="text-[11px] text-stone-400 hover:text-red-500 dark:hover:text-red-400 transition-colors">
                    Clear selection
                  </button>
                )}
              </div>
              <div className="bg-stone-50 dark:bg-[#13151f] rounded-2xl p-4 border border-stone-200 dark:border-[#2a2d3e]">
                <BookingCalendar
                  ...
                  onSelect={handleSelectDay} onHover={setHoverDate}
                  colors={tealColors}
                />
                <CalLegend items={[
            ...
            {stepMessage.phase < 3 ? (
              <div className="flex items-center gap-3 bg-teal-50 dark:bg-teal-950/20 border border-teal-200 dark:border-teal-800 rounded-xl px-4 py-3">
                {[1, 2].map(n => (
                  <div key={n} className={cn(
                    ...
                    stepMessage.phase === n ? "bg-teal-600 text-white" : "bg-stone-200 dark:bg-stone-700 text-stone-500 dark:text-stone-400"
                  )}>{n}</div>
                ))}
                <div className="w-8 h-0.5 bg-teal-200 dark:bg-teal-800 flex-shrink-0 -mx-1" />
                <p className="text-sm text-teal-700 dark:text-teal-300 font-medium">{stepMessage.text}</p>
              </div>
            ) : startDate && endDate && (
              <div className="bg-teal-50 dark:bg-teal-950/20 border border-teal-200 dark:border-teal-800 rounded-xl px-4 py-3.5 flex flex-col gap-2">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <p className="text-[10px] font-bold text-teal-600 dark:text-teal-500 uppercase tracking-widest mb-1">Check-in</p>
                    <p className="text-sm font-bold text-stone-800 dark:text-stone-100">{fmtDate(startDate)}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-bold text-teal-600 dark:text-teal-500 uppercase tracking-widest mb-1">Check-out</p>
                    <p className="text-sm font-bold text-stone-800 dark:text-stone-100">{fmtDate(endDate)}</p>
                  </div>
                </div>
                <div className="border-t border-teal-200 dark:border-teal-800 pt-2 flex items-center justify-between">
                  <span className="text-xs text-stone-500 dark:text-stone-400">
                    {numNights} night{numNights !== 1 ? "s" : ""} · {phpFmt.format(listingPrice)} {priceUnit}
                  </span>
                  {estimatedTotal !== null && (
                    <span className="text-sm font-bold text-stone-900 dark:text-stone-50">Est. {phpFmt.format(estimatedTotal)}</span>
                  )}
                </div>
              </div>
            )}
            ...
            <div>
              <label className="text-[11px] font-bold text-stone-500 dark:text-stone-400 uppercase tracking-widest mb-2 block">
                Message <span className="font-normal normal-case tracking-normal text-stone-400 dark:text-stone-500">(optional)</span>
              </label>
              <textarea
        ...
        <div className="px-5 pb-6 pt-3 flex gap-2.5 flex-shrink-0 border-t border-stone-100 dark:border-[#252837]">
          ...
          <button type="button" onClick={handleSend} disabled={!canSend} className="flex-1 py-3 rounded-full bg-teal-700 hover:bg-teal-600 text-white text-sm font-bold transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
...
export function BookServiceModal({
  ...
  const { getSchedule, createBooking } = useSchedule();
  const schedule = getSchedule(listingId);
  const isUnavailable = makeUnavailabilityChecker(schedule, fallbackSvcUnavailable);
  ...
  function handleSend() {
    if (!selectedDate) {
      toast.error("Please select a date for the service.", { position: "top-center" }); return;
    }
    if (!selectedWindow) {
      toast.error("Please choose a preferred time window.", { position: "top-center" }); return;
    }
    setSending(true);

    // If schedule exists, create booking
    if (schedule) {
      const booked = createBooking(listingId, selectedDate, buyerName, selectedWindow, message || undefined);
      if (!booked) {
        toast.error("That date is no longer available. Please pick another.", { position: "top-center" });
        setSending(false);
        return;
      ...
      <div className="bg-white dark:bg-[#1c1f2e] rounded-t-3xl sm:rounded-2xl w-full sm:max-w-md shadow-2xl flex flex-col max-h-[92vh] sm:max-h-[88vh]">
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="p-5 flex flex-col gap-5">
            ...
            {/* Calendar */}
            <div>
              <p className="text-[11px] font-bold text-stone-500 dark:text-stone-400 uppercase tracking-widest mb-3">Select a Date</p>
              <div className="bg-stone-50 dark:bg-[#13151f] rounded-2xl p-4 border border-stone-200 dark:border-[#2a2d3e]">
                <BookingCalendar
                  viewYear={viewYear} viewMonth={viewMonth}
                  onPrevMonth={prevMonth} onNextMonth={nextMonth}
                  isUnavailable={isUnavailable}
                  startDate={selectedDate} endDate={null} hoverDate={null}
            ...
            {selectedDate ? (
              <div className="flex items-center justify-between bg-violet-50 dark:bg-violet-950/20 border border-violet-200 dark:border-violet-800 rounded-xl px-4 py-3">
                ...
                <button type="button" onClick={() => setSelectedDate(null)} className="text-violet-400 hover:text-violet-600 dark:hover:text-violet-200 transition-colors">
                  <X className="w-4 h-4" />
                </button>
              </div>
            ) : (
            ...
            <div>
              ...
              {/* Show seller's hours if schedule available */}
              {schedule && (
                <p className="text-[11px] text-stone-400 dark:text-stone-500 mb-2">
                  Seller hours: {schedule.operatingStart}–{schedule.operatingEnd}
                </p>
              )}
            ...
            <div className="flex items-center justify-between bg-stone-50 dark:bg-[#13151f] border border-stone-200 dark:border-[#2a2d3e] rounded-xl px-4 py-3">
              ...
              <span className="text-sm font-bold text-stone-900 dark:text-stone-50">
                ...
                {priceUnit && <span className="text-xs font-normal text-stone-400 dark:text-stone-500"> {priceUnit}</span>}
              </span>
            </div>
        ...
        <div className="px-5 pb-6 pt-3 flex gap-2.5 flex-shrink-0 border-t border-stone-100 dark:border-[#252837]">
          ...
          <button type="button" onClick={handleSend} disabled={!canSend} className="flex-1 py-3 rounded-full bg-violet-700 hover:bg-violet-600 text-white text-sm font-bold transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
