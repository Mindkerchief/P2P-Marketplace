...
export default function ListingPage() {
  ...
  const [shownContactNumber, setShownContactNumber] = useState<string | null>(
    null,
  );
  ...
  const visitorUnavailableState =
    isUnavailableState ||
    isBannedState ||
    isDeletedState ||
    isSellerInactiveState;
  const isSold =
    isSell && (listingStatus === 'sold' || listingSellStatus === 'sold');
  const isListingAvailable = listingStatus === 'available';
  const images = galleryImageUrls;
  const sellerRating = Number.isFinite(listing.seller.rating)
    ? Number(listing.seller.rating)
    : 0;
  const hasSellerRating = sellerRating > 0;
  const sellerProfileHref = isOwnListing
    ? '/profile'
    : listing.seller.id
      ? `/profile?userId=${listing.seller.id}`
      : '/profile';
  const sellerOnline = listing.seller.id
    ? isUserOnline(listing.seller.id)
    : false;
    ...
    try {
      ...
      const message =
        err instanceof Error
          ? err.message
          : String(err || 'Failed to submit report.');
  ...
  async function sendOffer() {
    ...
    try {
      const conversationId = await openOrCreateConversationFromListing(
        listing.id,
        parsedOffer,
        offerMessage,
      );
                  ...
                  {images.map((img, i) => (
                    <Button
                      ...
                      className={cn(
                        ...
                        i === imgIdx
                          ? 'border-slate-800 dark:border-stone-300'
            ...
            <div className="bg-white dark:bg-[#1c1f2e] rounded-lg border border-stone-200 dark:border-[#2a2d3e] shadow-sm divide-y divide-stone-100 dark:divide-[#2a2d3e]">
              {[
                ...
                },
                {
                  ...
                  value:
                    extra.transactionCount > 1
                      ? `${extra.transactionCount} Interactions`
                ...
                },
                {
                  ...
                  value:
                    extra.reviewCount > 1
                      ? `${extra.reviewCount} Reviews`
                      : `${extra.reviewCount} Review`,
                },
              ].map((row) => (
                <div
                  key={row.label}
