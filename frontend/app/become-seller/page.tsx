...
async function fileToVerificationImagePayload(
  file: File,
  fieldName: string,
...
export default function BecomeSellerPage() {
  ...
  useEffect(() => {
    if (!showOtp || otpComplete === false || verifyingOtp || otpVerified)
      return;
  ...
  function validateStep1(): string | null {
    if (!agreed)
      return 'You need to agree to the Seller Terms of Service first.';
    ...
    if (
      !city ||
      city.length < LISTING_LIMITS.locationMinLength ||
      city.length > LISTING_LIMITS.locationMaxLength
    ) {
      return `Profile city must be between ${LISTING_LIMITS.locationMinLength} and ${LISTING_LIMITS.locationMaxLength} characters.`;
    }
    if (
      !province ||
      province.length < LISTING_LIMITS.locationMinLength ||
      province.length > LISTING_LIMITS.locationMaxLength
    ) {
  ...
  function validateStep2(): string | null {
    ...
    if (
      !VERIFICATION_ID_TYPES.includes(
        normalizedIdType as (typeof VERIFICATION_ID_TYPES)[number],
      )
    ) {
    ...
    if (!(idFront instanceof File))
      return 'Front photo of your ID is required.';
    if (!(idBack instanceof File)) return 'Back photo of your ID is required.';
    if (!(selfie instanceof File))
      return 'Selfie while holding your ID is required.';
    return null;
  }
              ...
              <div className="space-y-5">
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      {isMobile
                        ? 'Smartphone detected'
                        : 'Smartphone with camera required'}
                    </p>
                    <p
                      className={cn(
                ...
                >
                  ...
                  <span className="text-sm text-stone-500 dark:text-stone-400 leading-relaxed">
                    ...
                    and confirm I am at least 18 years old and a resident of the
                    Philippines.
                  </span>
                </label>
              </div>
