...
export default function ReportsPage() {
  ...
  const loadReports = useCallback(
    async (pageNumber: number) => {
      ...
      try {
        ...
        const message =
          typeof err === 'string' ? err : 'Failed to load reports';
  ...
  const reasonOptions = useMemo(() => {
    const uniqueReasons = Array.from(
      new Set(reports.map((r) => r.reason).filter(Boolean)),
    );
  ...
  async function handleModalSubmit(
    ...
    if (trimmedReason.length > REPORT_ACTION_REASON_MAX_LENGTH) {
      toast.error(
        `Reason must not exceed ${REPORT_ACTION_REASON_MAX_LENGTH} characters`,
        { position: 'top-center' },
      );
      return;
    }
    ...
    try {
      ...
      const message =
        typeof err === 'string' ? err : 'Failed to submit report action';
      toast.error(message, { position: 'top-center' });
    } finally {
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        >
          <RotateCw
            className={cn(
              'w-3.5 h-3.5',
              loadingReports && isRefreshing && 'animate-spin',
            )}
          />{' '}
          Refresh
        </Button>
