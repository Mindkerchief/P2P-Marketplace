...
export default function ListingsPage() {
  ...
  const [actionLoadingListingId, setActionLoadingListingId] = useState<
    string | null
  >(null);
  ...
  const loadListings = useCallback(
    async (pageNumber: number) => {
      ...
      try {
        ...
        const message =
          typeof err === 'string' ? err : 'Failed to load listings';
  ...
  const categoryOptions = useMemo(() => {
    const unique = Array.from(
      new Set(listings.map((l) => l.category.trim()).filter(Boolean)),
    );
  ...
  const filtered = useMemo(() => {
    ...
    data.sort((a, b) => {
      ...
      if (sort.field === 'title') {
        ...
        va = a.price;
        vb = b.price;
      } else if (sort.field === 'transactions') {
        va = a.transaction_count;
        vb = b.transaction_count;
      ...
      } else if (sort.field === 'bannedUntil') {
        va = a.banned_until ? new Date(a.banned_until).getTime() : 0;
        vb = b.banned_until ? new Date(b.banned_until).getTime() : 0;
      } else if (sort.field === 'deletedAt') {
      ...
      onConfirm: () => {
        void (async () => {
          ...
          try {
            ...
            toast.success('Listing marked as deleted.', {
              position: 'top-center',
            });
          } catch (err) {
            const message =
              typeof err === 'string' ? err : 'Failed to delete listing';
            toast.error(message, { position: 'top-center' });
          } finally {
  ...
  async function handleToggleVisibility(
    ...
    if (currentStatus === 'DELETED') {
      toast.error('Cannot update visibility for deleted listing', {
        position: 'top-center',
      });
      return;
    }
              ...
              prev.map((listing) =>
                listing.id === id
                  ? {
                      ...
                      action_by_name:
                        actionByNamePlaceholder || listing.action_by_name,
                    }
                  : listing,
              ),
            );
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        >
          <RotateCw
            className={cn(
              'w-3.5 h-3.5',
              loadingListings && isRefreshing && 'animate-spin',
            )}
          />{' '}
          Refresh
        </Button>
