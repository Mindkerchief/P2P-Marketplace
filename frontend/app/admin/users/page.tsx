...
export default function UsersPage() {
  ...
  const filtered = useMemo(() => {
    ...
    data.sort((a, b) => {
      ...
      } else if (sort.field === 'locked_until') {
        va = a.account_locked_until
          ? new Date(a.account_locked_until).getTime()
          : 0;
        vb = b.account_locked_until
          ? new Date(b.account_locked_until).getTime()
          : 0;
  ...
  async function handleToggleActive(id: string) {
    ...
    const isCurrentlyBanned = !!(
      target.account_locked_until &&
      new Date(target.account_locked_until) > new Date()
    );
    ...
    openDialog({
      ...
      message: isCurrentlyBanned
        ? 'Unban this user account?'
        : 'Ban this user for 3 days?',
      ...
      onConfirm: () => {
        void (async () => {
          ...
          try {
              ...
              prev.map((user) =>
                user.id === id
                  ? {
                      ...
                      action_by_name: nextIsActive
                        ? ''
                        : actor?.fullName || user.action_by_name || '',
                      action_by_email: nextIsActive
                        ? ''
                        : actor?.email || user.action_by_email || '',
                      updated_at: nowIso,
                    }
                  : user,
              ),
            ...
            toast.success(
              `User ${nextIsActive ? 'unbanned' : 'banned for 3 days'} successfully`,
              { position: 'top-center' },
            );
          } catch (err) {
            const message =
              typeof err === 'string' ? err : 'Failed to update user status';
            toast.error(message, { position: 'top-center' });
          } finally {
      ...
      onConfirm: () => {
        void (async () => {
          ...
          try {
            ...
            const message =
              typeof err === 'string' ? err : 'Failed to delete user';
            toast.error(message, { position: 'top-center' });
          } finally {
            setActionLoadingUserId(null);
          }
  ...
  const hasActiveFilters =
    search || verifFilter !== 'ALL' || statusFilter !== 'ALL';
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        >
          <RotateCw
            className={cn(
              'w-3.5 h-3.5',
              loadingUsers && isRefreshing && 'animate-spin',
            )}
          />{' '}
          Refresh
        </Button>
        ...
        <CardContent className="p-0 h-full min-h-0 flex flex-col">
          ...
          <div className="px-4 py-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between text-sm text-stone-400 dark:text-stone-500">
            <span>
              ...
              {totalUsersCount.toLocaleString()} result
              {totalUsersCount !== 1 ? 's' : ''}
            </span>
