...
export default function ProfilePage() {
  ...
  const isSelfProfileRequest =
    externalUserId !== '' && ownUserId !== '' && externalUserId === ownUserId;
  const isViewingExternalProfile =
    externalUserId !== '' && !isSelfProfileRequest;
  const [profileUser, setProfileUser] = useState<ProfilePayload['user'] | null>(
    null,
  );
  ...
  const [editSnapshot, setEditSnapshot] =
    useState<EditableProfileSnapshot | null>(null);
  ...
  const [loadingMoreReceivedReviews, setLoadingMoreReceivedReviews] =
    useState(false);
  const [loadingMorePersonalReviews, setLoadingMorePersonalReviews] =
    useState(false);
  ...
  const handleAvatarUpload = async (file: File) => {
    ...
    try {
      const profileImage = await encodeSquareProfileImageToPayload(
        file,
        'profileImage',
      );
  ...
  const handleCoverUpload = async (file: File) => {
    ...
    try {
      const coverImage = await encodeImageToPayload(
        file,
        'cover',
        'coverImage',
      );
    ...
    const loadProfile = async () => {
      ...
      try {
        ...
        setBookmarksTotal(
          isViewingExternalProfile
            ? 0
            : (payload.bookmarksTotal ?? payload.bookmarks.length),
        );
        setReceivedReviewsTotal(
          payload.receivedReviewsTotal ??
            (payload.receivedReviews ?? payload.reviews ?? []).length,
        );
        setPersonalReviewsTotal(
          payload.personalReviewsTotal ??
            (payload.personalReviews ?? []).length,
        );
        if (!isViewingExternalProfile) {
  ...
  const visibleUserListings = userListings.filter(
    (l) => (l.status ?? '').toLowerCase() !== 'deleted',
  );
  const soldListings = visibleUserListings.filter((l) =>
    SOLD_STATUSES.has((l.status ?? '').toLowerCase()),
  );
  ...
  const allListings: ProfileListingItem[] =
    listingTab === 'all'
      ? visibleUserListings
      : listingTab === 'active'
        ? activeListings
  ...
  async function loadMoreReviews(target: ReviewTab) {
    ...
    if (target === 'received') {
      ...
      try {
        ...
        setReceivedReviewsTotal(
          payload.receivedReviewsTotal ?? receivedReviewsTotal,
        );
      } catch {
    ...
    try {
      ...
      setPersonalReviewsTotal(
        payload.personalReviewsTotal ?? personalReviewsTotal,
      );
    } catch {
      showErrorToast('Failed to load more reviews');
    } finally {
  ...
  async function handleSave() {
    ...
    if (form.bio.trim().length > AUTH_LIMITS.profileBioMaxLength) {
      showErrorToast(
        `Bio must not exceed ${AUTH_LIMITS.profileBioMaxLength} characters`,
      );
      return;
    }
    ...
    if (normalizedPhone !== '') {
      if (normalizedPhone.length !== AUTH_LIMITS.phoneLength) {
        showErrorToast(
          `Phone number must be exactly ${AUTH_LIMITS.phoneLength} digits`,
        );
        return;
      }
    ...
    if (form.currentPassword.length > AUTH_LIMITS.passwordMaxLength) {
      showErrorToast(
        `Current password must not exceed ${AUTH_LIMITS.passwordMaxLength} characters`,
      );
      return;
    }
    ...
    if (form.newPassword.length > AUTH_LIMITS.passwordMaxLength) {
      showErrorToast(
        `New password must not exceed ${AUTH_LIMITS.passwordMaxLength} characters`,
      );
      return;
    }
    ...
    if (form.confirmPassword.length > AUTH_LIMITS.passwordMaxLength) {
      showErrorToast(
        `Confirm password must not exceed ${AUTH_LIMITS.passwordMaxLength} characters`,
      );
      return;
    }
    ...
    if (
      form.newPassword &&
      form.newPassword.length < AUTH_LIMITS.passwordMinLength
    ) {
      showErrorToast(
        `New password must be at least ${AUTH_LIMITS.passwordMinLength} characters`,
      );
      return;
    }
  ...
  const fullName = profileUser
    ...
    : '—';
  const locationText =
    [profileUser?.locationCity, profileUser?.locationProv]
      .filter(Boolean)
  ...
  const overallRatingText = formatOverallRating(
    profileUser?.overallRating,
    profileUser?.reviewCount,
  );
  const hasOverallRating = (profileUser?.reviewCount ?? 0) > 0;
  const isVerifiedSeller =
    !isViewingExternalProfile &&
    (profileUser?.status ?? '').toLowerCase() === 'verified';
  const profileTargetUserId = isViewingExternalProfile
    ? externalUserId
    : (user?.userId ?? '');
  const isProfileOnline = profileTargetUserId
    ? isUserOnline(profileTargetUserId)
    : false;

  // Shared label style
  const lbl =
    'text-xs font-medium text-stone-600 dark:text-stone-400 mb-1.5 block';
    ...
    <div className="min-h-fit bg-[#faf6f0] dark:bg-[#111827]">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="bg-white dark:bg-[#1c1f2e] rounded-lg border border-stone-200 dark:border-[#2a2d3e] shadow-sm overflow-hidden mb-4">
          ...
          <div
            className={cn(
              'relative min-h-36 bg-linear-to-r from-[#1a0e00] via-[#1a2235] to-[#0b0f1a] overflow-hidden group',
              !isViewingExternalProfile && 'cursor-pointer',
            )}
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm({
                        ...form,
                        firstName: e.target.value.slice(
                          0,
                    ...
                    onChange={(e) =>
                      setForm({
                        ...
                        bio: e.target.value.slice(
                          0,
                          AUTH_LIMITS.profileBioMaxLength,
                        ),
                      })
                    }
                      ...
                      <div className="relative">
                        <Input
                          ...
                          onChange={(e) =>
                            setForm({
                              ...form,
                              currentPassword: e.target.value.slice(
                                0,
                      ...
                      <div className="relative">
                        <Input
                          ...
                          onChange={(e) =>
                            setForm({
                              ...form,
                              newPassword: e.target.value.slice(
                                0,
                                AUTH_LIMITS.passwordMaxLength,
                              ),
                            })
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setForm({
                            ...form,
                            confirmPassword: e.target.value.slice(
                              0,
                              AUTH_LIMITS.passwordMaxLength,
                            ),
                          })
                ...
                <div className="flex gap-1">
                  {(['all', 'active', 'sold', 'booked'] as const).map((tab) => (
                    <button
                      ...
                      className={cn(
                        'tab-page-base',
                        listingTab === tab ? 'tab-active' : 'tab-inactive',
                      )}
                    >
                      {tab === 'all'
                ...
                <div className="flex gap-1">
                  {(
                    [
                      {
                        key: 'received',
                        label: `Reviews by Others (${receivedReviews.length})`,
                      },
                      {
                        key: 'personal',
                        label: `Personal Reviews (${personalReviews.length})`,
                      },
                    ] as const
                  ).map((tabItem) => (
                    <button
